        ...
        if ( !empty(config('wk-core.class.site.site')) ) {
            $validator->after(function ($validator) {
                ...
                if (isset($data['host_type']) && isset($data['host_id'])) {
                    if ( config('wk-mall-order.onoff.site') && !empty(config('wk-core.class.site.site')) && $data['host_type'] == config('wk-core.class.site.site') ) {
                ...
                if (isset($data['channel_id'])) {
                    if ( config('wk-mall-order.onoff.mall-cart') && !empty(config('wk-core.class.mall-cart.channel')) ) {
                ...
                if (isset($data['data'])) {
                    ...
                    if ($obj->grandtotal < 0 || $obj->subtotal_discount < 0 || $obj->fee < 0 || $obj->tax < 0 || $obj->tip < 0)
                        $validator->errors()->add('data', trans('php-core::validation.not_allowed'));
                    elseif ($obj->discount_coupon > 0 || $obj->discount_point > 0)
                        $validator->errors()->add('data', trans('php-core::validation.not_allowed'));
                    ...
                    foreach ($obj->items as $items) {
                        foreach ($items as $item) {
                            ...
                            if (empty($item->binding)) continue;
